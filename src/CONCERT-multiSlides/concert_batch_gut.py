...
class CONCERT(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.lord_encoder = Lord_encoder(
            ...
            labels=cell_atts,
            attributes=["perturbation", "day"],
            attributes_type=["categorical", "ordinal"],
            noise=self.noise,
            device=device,
        )
    ...
    def forward(
        self,
        *,
        x: torch.Tensor,               # positions with appended batch one-hot (N, 2 + n_batch)
        y: torch.Tensor,               # normalized counts
        batch: torch.Tensor,           # batch one-hot (N, n_batch)
        raw_y: torch.Tensor,           # raw counts
        sample_index: torch.Tensor,    # indices for LORD basal embedding
        cell_atts: torch.Tensor,       # [perturb_code, day_numeric]
        size_factors: torch.Tensor,    # NB size factors
        num_samples: int = 1,
    ):
        self.train()
        bsz = y.shape[0]
        ...
        for _ in range(max(1, num_samples)):
            ...
            if self.shared_dispersion:
                ...
                # per-batch dispersion: (G, B) · (N, B)^T → (N, G), via matmul with one-hot
        ...
        elbo = recon_loss + self.beta * (gp_KL_term + gaussian_KL_term) + lord_loss
        ...
        return (
            ...
            inside_elbo_recon,
            inside_elbo_kl,
            None,                      # latent_samples (omit to save mem)
    ...
    def counterfactualPrediction(
        ...
        *,
        ...
        target_cell_day: Optional[float] = None,
        target_cell_perturbation: Optional[int] = None,
        n_samples: int = 1,
        batch_size: int = 512,
    ) -> Tuple[np.ndarray, np.ndarray]:
        """Counterfactual counts after modifying selected cells' (day, perturbation)."""
        self.eval()
        X_t = torch.tensor(X, dtype=self.dtype)
        idx_t = torch.tensor(sample_index, dtype=torch.int)
        ...
        if perturb_cell_id is not None:
            ids = perturb_cell_id.tolist() if isinstance(perturb_cell_id, np.ndarray) else list(perturb_cell_id)
            for i in ids:
                if target_cell_perturbation is not None:
                    pert_atts[:, 0][i] = int(target_cell_perturbation)
                if target_cell_day is not None:
                    pert_atts[:, 1][i] = float(target_cell_day)
